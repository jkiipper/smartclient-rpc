...
class DataSourceFactory extends ResourceFactory {
    ...
    create() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            XMLUtil.readXMLFile(path.join(process.cwd(), self._path, self._dsName + Const.DATA_SOURCE_FILE_EXTENTION), function(err, config) {
                if (err) {
                    return reject(new Exception("Failed to read data source '" + self._dsName + "' configuration", err));
                }
                if (self._dsName !== config.ID) {
                    return reject(new Exception("Data source name mismatch. Looking for '" + self._dsName + "', but got '" + config.ID + "'"));
                }
                let ds;
                if (config.serverConstructor) {
                    try {
                        ...
                        if (!(ds instanceof DataSource)) {
                            return reject(new Exception("'" + self._dsName + "' is not instance of DataSource"));
                        }
                        return resolve(ds);
                    } catch (err) {
                        return reject(new Exception("Failed to intantiate data source '" + self._dsName + "'", err));
                    }
                }
                ...
                if (!ds) {
                    return reject(new Exception("Unknown data source type '" + dsType + "'"));
                }
                resolve(ds);
            });
        });
