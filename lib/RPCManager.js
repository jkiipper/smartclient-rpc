...
class RPCManager {
    ...
    processRequest() {
        ...
        this._parseRequest(function(err) {
            ...
            if (self.log.isDebugEnabled) {
                const debugOp = [];
                for (let i = 0, l = self.operations.length; i < l; i++) {
                    debugOp.push(self.operations[i].toObject());
                }
                self.log.debug({operations: debugOp}, "Processing operations");
            }
            const responses = [];
            self._initRequests(self.operations, function(err) {
                if (err) {
                    // Failed to initialize operations - free resources and send top level error
                    return self._freeResources(self.operations, function() {
                        // Send top level error
                        self.sendResponse(err);
                    });
                }
                self._execute(self.operations, function(err, results) {
                    if (err) {
                        // Should never occur because _execute() always returns results array with
     ...
     * @param {function} callback - Callback executed when finished
     */
    _initRequests(operations, callback) {
        if (!operations) {
        ...
        Util.arrayExecutor(operations, true, function(index, op, cb) {
    ...
    _execute(operations, callback) {
        ...
        Util.arrayExecutor(operations, false, function(index, op, cb) {
            if (op instanceof BaseRequest) {
     ...
     * 
     * @param {*|Array} operations - Operations to free resources
     * @param {function} callback - Callback executed when finished
     */
    _freeResources(operations, callback) {
        const self = this;
        if (!operations) {
            return callback();
        }
